	...
	try {
		...
		await sock.sendMessage(
			chatId,
			{
				...
				caption: `ðŸŽµ Downloading: *${audioData.title || video.title}*\nâ± Duration: ${audioData.duration || video.timestamp || 'Unknown'}\nðŸŽ§ Quality: ${audioData.quality || 'Unknown'}`
			},
			{ quoted: message }
		);
		...
		await sock.sendMessage(
			chatId,
			{ text: 'âŒ Failed to download song.' },
			{ quoted: message }
		);
	}
}
